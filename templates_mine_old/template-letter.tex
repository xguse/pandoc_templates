\documentclass[$if(fontsize)$$fontsize$,$endif$$if(lang)$$lang$,$endif$$if(papersize)$$papersize$,$endif$$for(classoption)$$classoption$$sep$,$endfor$]{letter}
$if(fontfamily)$
...
$if(closing)$
$if(fromsig)$
% Move signature and close to the left
\longindentation=0pt

\closing{$closing$\\
\vspace{12pt}
\fromsig{\includegraphics[height=36pt]{$fromsig$}} \\
\fromname{$author$}
}
$else$
% Move signature and close to the left
